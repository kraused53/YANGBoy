...
SM83::SM83(void) {
  ...
  opcodes = {
      ...
      {&s::LD_mRP_SP, RG_NONE, RG_NONE, CN_NONE,
       0},                                          // 0x08 - LD [a16], regs.sp
      ...
      {&s::LD_RP_IMM, RP_SP, RG_NONE, CN_NONE, 0},    // 0x31 - LD regs.sp, d16
      {&s::LD_mRP_A, RP_HL, RG_NONE, CN_NONE, 0},     // 0x32 - LD [HL-], A
      {&s::INC_RP, RP_SP, RG_NONE, CN_NONE, 0},       // 0x33 - INC regs.sp
      ...
      {&s::ADD_HL_RP, RG_NONE, RP_SP, CN_NONE, 0},    // 0x39 - ADD HL, regs.sp
      {&s::LD_A_mRP, RG_NONE, RP_HL, CN_NONE, 0},     // 0x3A - LD A, [HL-]
      {&s::DEC_RP, RP_SP, RG_NONE, CN_NONE, 0},       // 0x3B - DEC regs.sp
      ...
      {&s::CB_OPCODES, RG_NONE, RG_NONE, CN_NONE,
       0},                                       // 0xCB - CB Prefix Opcodes
      ...
      {&s::ADD_SP_IMM, RG_NONE, RG_NONE, CN_NONE, 0},  // 0xE8 - ADD regs.sp, d8
      ...
      {&s::ADD_SP_IMM, RG_NONE, RG_NONE, CN_NONE,
       0},  // 0xF8 - LD HL, regs.sp + d8
      {&s::LD_SP_HL, RG_NONE, RG_NONE, CN_NONE, 0},    // 0xF9 - LD regs.sp, HL
...
/* Memory */
uint8_t SM83::read_byte(uint16_t addr) { return bus->read(addr); }
...
uint8_t SM83::fetch_byte(void) {
  uint8_t data = read_byte(regs.pc);
  regs.pc++;
  return data;
}
...
void SM83::write_byte(uint16_t addr, uint8_t data) { bus->write(addr, data); }
...
void SM83::set_flag(FLAG f, bool val) {
  ...
  // Always clear low nibble
  regs.f &= 0xF0;
}

bool SM83::get_flag(FLAG f) { return regs.f & f; }
...
uint16_t SM83::pop_from_stack(void) {
  uint16_t ret = read_word(regs.sp);
  regs.sp += 2;
  return ret;
}
...
void SM83::push_to_stack(uint16_t data) {
  regs.sp -= 2;
  write_word(regs.sp, data);
}

void SM83::DBG(void) {
  // A:00 F:11 B:22 C:33 D:44 E:55 H:66 L:77 regs.sp:8888 PC:9999
  // PCMEM:AA,BB,CC,DD
  printf(
      "A:%02X F:%02X B:%02X C:%02X D:%02X E:%02X H:%02X L:%02X regs.sp:%04X "
      "PC:%04X "
      "PCMEM:%02X,%02X,%02X,%02X\n",
      regs.a, regs.f, regs.b, regs.c, regs.d, regs.e, regs.h, regs.l, regs.sp,
      regs.pc, read_byte(regs.pc), read_byte(regs.pc + 1),
      read_byte(regs.pc + 2), read_byte(regs.pc + 3));
  /*
  printf( "PC: %04X, AF: %04X, BC: %04X, DE: %04X, HL: %04X, regs.sp: %04X, CYC:
  %d", PC, regs.( RP_AF ), regs.( RP_BC ), regs.( RP_DE ), regs.( RP_HL ),
  regs.sp, cycles );
...
void SM83::INC_RG(void) {
  ...
  if (opcode == 0x34) {
    result = read_byte(regs.hl) + 1;
    write_byte(regs.hl, result);
    cycles = 12;
  } else {
    switch (GET_DST) {
      case RG_B:
...
void SM83::ADD_HL_RP(void) {
  ...
  set_flag(F_N, false);
  set_flag(F_H, will_carry(12, regs.hl, src, false));
...
void SM83::JUMP_REL(void) {
  // Fetch regs.a byte and convert it to signed integer
...
void SM83::DAA(void) {
  if (get_flag(F_N)) {
    ...
    uint16_t tmp_word = regs.a;

    if (((regs.a & 0x0F) > 9) || get_flag(F_H)) {
      tmp_word += 0x06;
    }
    ...
    regs.a = uint8_t(tmp_word);
  }

  set_flag(F_Z, regs.a == 0x00);
  set_flag(F_H, false);
...
void SM83::ADD(void) {
  ...
  regs.a = internal_add(regs.a, to_op, false);
  set_flag(F_N, false);
}
...
void SM83::ADC(void) {
  ...
  regs.a = internal_add(regs.a, to_op, get_flag(F_C));
  set_flag(F_N, false);
}
...
void SM83::SUB(void) {
  ...
  regs.a = internal_add(regs.a, ~to_op, true);
  set_flag(F_C, !get_flag(F_C));
...
void SM83::CP(void) {
  ...
  uint8_t tmp_byte = (uint8_t)(regs.a - to_op);

  set_flag(F_Z, tmp_byte == 0x00);
  set_flag(F_N, true);
  set_flag(F_H, (int)((regs.a & 0x0F) - (tmp_byte & 0x0F)) < 0);
  set_flag(F_C, regs.a < to_op);
}
...
void SM83::LDH(void) {
  ...
  switch (opcode) {
    case 0xE0:
      offset = fetch_byte();
      write_byte((0xFF00 + offset), regs.a);
      cycles = 12;
      break;
    case 0xE2:
      write_byte((0xFF00 + regs.c), regs.a);
      cycles = 16;
      break;
    case 0xF0:
      offset = fetch_byte();
      regs.a = read_byte((0xFF00 + offset));
      cycles = 12;
      break;
    case 0xF2:
...
void SM83::ADD_SP_IMM(void) {
  int8_t offset = (int8_t)fetch_byte();
  int res = int(regs.sp + offset);

  set_flag(F_Z, false);
  set_flag(F_N, false);
  set_flag(F_H, ((regs.sp ^ offset ^ (res & 0xFFFF)) & 0x10) == 0x10);
  set_flag(F_C, ((regs.sp ^ offset ^ (res & 0xFFFF)) & 0x100) == 0x100);
...
void SM83::RES(void) {
  if (((opcode & 0x0F) == 0x06) || ((opcode & 0x0F) == 0x0E)) {
    cycles = 12;
    write_byte(regs.hl, read_byte(regs.hl) & ~(1 << CB_GET_OPT));
  } else {
    cycles = 8;
    switch (CB_GET_DST) {
      case RG_B:
